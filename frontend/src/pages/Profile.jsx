...
const calculateStats = (hikes) => {
  ...
  hikes.forEach((hike, index) => {
    console.log(`\n=== Processing Hike ${index + 1} ===`);
    console.log('Hike object:', hike);
    
    // Sum distances (assuming distance is in km, convert to miles: 1 km = 0.621371 miles)
    const distance = parseFloat(hike.distance) || 0;
    console.log('Distance:', distance, 'km →', (distance * 0.621371).toFixed(2), 'miles');
    totalMiles += distance ;
    ...
    console.log('Difficulty raw:', difficultyRaw, 'Type:', typeof difficultyRaw);
...
const ProfilePage = () => {
  ...
  const user = {
    ...
    bio: "Adventure seeker exploring trails across the Rocky Mountains. Nature photographer and weekend warrior.",
    joinedDate: formatJoinDate(currentUser?.createdAt),
    avatar: currentUser?.picture || defaultUserProfile,
  };

  // Fetch completed hikes and calculate statistics
  useEffect(() => {
    const fetchHikesAndCalculateStats = async () => {
      if (currentUser?.id) {
        setLoading(true);
        try {
          ...
          console.log('=== RAW HIKES DATA ===');
          console.log('Full response:', data);
          console.log('Hikes array:', hikes);
          console.log('Number of hikes:', hikes.length);
          
          // DEBUG: Log each hike's structure
          if (hikes.length > 0) {
            console.log('=== FIRST HIKE STRUCTURE ===');
            console.log('First hike:', hikes[0]);
            console.log('Available keys:', Object.keys(hikes[0]));
            console.log('Difficulty field:', hikes[0].difficulty);
            console.log('Distance field:', hikes[0].distance);
            console.log('Duration field:', hikes[0].duration);
            console.log('Elevation field:', hikes[0].elevation);
          }
        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-6">
              <Avatar src={user.picture} fallback="SM" className="h-28 w-28" />
              <div>
